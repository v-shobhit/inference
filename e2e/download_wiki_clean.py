...
class WikipediaCleanExtractor:
    ...
    def get_article_content(self, title: str, timeout: int = 30,
    ...
    def format_headings(self, text: str) -> str:
        ...
        for i, line in enumerate(lines):
            ...
            if is_short and no_punctuation and not_all_lower and (
                    prev_empty or next_empty):
                # Check if it's not a list item or sentence fragment
                if not stripped.startswith(
                        ('•', '-', '*', '1.', '2.', '3.', '(')) and ' ' in stripped:
    ...
    def clean_wikipedia_text(self, text: str) -> str:
        ...
        # Note: API returns plain text, so sections appear as plain headings,
        # not wiki markup
        section_patterns = [
            r'\n+See also\s*\n.*$',
            r'\n+References\s*\n.*$',
        ...
        if i == 0:
            ...
            if len(chunk_words) > max_chunk_size:
                overlapped_chunks.append(
                    ' '.join(chunk_words[:max_chunk_size]))
            else:
            ...
            if len(combined_words) > max_chunk_size:
                overlapped_chunks.append(
                    ' '.join(combined_words[:max_chunk_size]))
            else:
        ...
        if use_semantic:
            try:
                ...
                if not hasattr(create_chunks_from_text, 'nlp'):
                    ...
                    create_chunks_from_text.embedding_model = SentenceTransformer(
                        "all-MiniLM-L6-v2")
                ...
                section_chunks = add_chunk_overlap(
                    section_chunks,
                    overlap_percentage=overlap_pct,
                    max_chunk_size=max_words)

                # Merge small chunks within section
                section_chunks = merge_small_chunks(
                    section_chunks, min_chunk_size=min_words, max_chunk_size=max_words)

            except Exception as e:
                if verbose:
                    print(
                        f"  Semantic chunking failed for section ({e}), falling back to simple chunking")
                section_chunks = []
...
def chunk_article_worker(
        args_tuple: Tuple) -> Tuple[Path, List[Dict[str, str]], Dict]:
    """
...
def create_passages_from_articles(
    ...
    print(
        f"Chunk size: {max_length} characters, Overlap: {overlap} characters")
...
def extract_wikipedia_links(item) -> List[str]:
    ...
    if isinstance(item, list):
        # Already a list
        links = [url for url in item if isinstance(
            url, str) and url.startswith('https://en.wikipedia.org/wiki/')]
        ...
        try:
            ...
            if isinstance(parsed, list):
                links = [
                    url for url in parsed if isinstance(
                        url, str) and url.startswith('https://en.wikipedia.org/wiki/')]
            elif isinstance(parsed, str) and parsed.startswith('https://en.wikipedia.org/wiki/'):
                links = [parsed]
        except (ValueError, SyntaxError) as e:
            print(
                f"Warning: Failed to parse wiki_links: {item[:100]}... Error: {e}")
            return []
...
def main():
    ...
    if args.chunk_workers > max_workers:
        print(
            f"⚠️  WARNING: {args.chunk_workers} chunk workers exceeds CPU count ({max_workers})!")
        print(f"   Reducing to {max_workers} workers...")
        args.chunk_workers = max_workers
    elif args.chunk_workers > 8:
        print(
            f"⚠️  Note: {args.chunk_workers} chunk workers may use significant memory (~500MB per worker)")
    ...
    with Pool(processes=args.download_workers) as pool:
        with tqdm(total=len(urls), desc="Downloading articles", unit="article") as pbar:
            # Use imap_unordered for immediate progress updates as workers
            # complete
            for result in pool.imap_unordered(
                    process_single_url, process_args):
    ...
    if args.create_chunks:
        ...
        if "error" not in stats:
            ...
            print(
                f"Average passages per article: {stats['avg_passages_per_article']:.1f}")

            # Show chunking method used
            if stats.get('semantic_chunking'):
                print(
                    f"Chunking method: SEMANTIC (similarity threshold: {stats['similarity_threshold']})")
            else:
        ...
        print(
            f"  python {os.path.basename(__file__)} --output_dir {output_dir} --create_chunks")
