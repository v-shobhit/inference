...
def create_vector_store_from_passages(
    ...
    # Note: We use .pop() to remove 'passage' key and keep everything else as
    # metadata
    ...
    print(f"\nTiming breakdown:")
    print(
        f"  Load passages:     {load_time:8.2f}s ({load_time/total_time*100:5.1f}%)")
    print(
        f"  Initialize model:  {init_time:8.2f}s ({init_time/total_time*100:5.1f}%)")
    print(
        f"  Ingest passages:   {ingest_time:8.2f}s ({ingest_time/total_time*100:5.1f}%)")
    print(
        f"  Serialize:         {serialize_time:8.2f}s ({serialize_time/total_time*100:5.1f}%)")
    print(f"  {'─' * 40}")
    ...
    print(
        f"  python batch_retrieval.py --vector_store {output_path} [other args...]")
