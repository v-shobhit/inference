...
class Pipeline:
    ...
    @classmethod
    def from_config(cls, config: PipelineConfig, verbose: bool = True) -> 'Pipeline':
        """
        ...
        rewriter, vector_store, engine, processor = builder.build_all(verbose=verbose)
    ...
    def run(self, prompts=None, verbose: bool = True) -> PipelineResult:
        ...
        # Process prompts
        df, io_data, stats = self.processor.process_prompts(prompts, proc_config)
    ...
    def _print_processing_header(self, num_prompts: int, proc_config: Dict[str, Any]) -> None:
        """
        Print processing header.
        ...
        print(f"Starting batch {mode_str} on {num_prompts} prompts{filtering_str}")
        print(f"{'='*80}\n")
